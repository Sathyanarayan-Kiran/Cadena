...
## 2026-09-24 — US13.4 work-note privacy in comment sync (Codex)

Public comments can now cross a managed Jira/ServiceNow counterpart without carrying private troubleshooting notes with them.

- **Off until enabled.** Every connector has its own comment policy, initially off. Its direction can read from that source, write to that source, or do both; a transfer needs both connectors to permit their half.
- **Private text stops at the provider edge.** The Jira adapter removes restricted-visibility and JSM-internal comments. The ServiceNow adapter removes `work_notes` and returns only the public `comments` journal. Cadena never receives the private body, so it cannot enter a queue, table, audit event or target request.
- **Original-author rule.** Optional allow and block lists compare stable provider account ids, case-insensitively, with the block list winning. Display names are retained only for attribution.
- **Attribution and no loops.** A transferred public comment visibly identifies the original author, account id and source. Its stable Cadena marker suppresses the copy when it returns; retries first look for that marker so a write that completed before a crash is not duplicated.
- **Read-only thread.** Open either synchronized twin to see the shared public thread. It is deliberately read-only; comments still originate in Jira or ServiceNow.
- **Late linking works.** If a public comment was ingested before its counterpart existed, the next source sync creates the missing delivery after the correlation is linked.

The story is verified by five focused acceptance tests and the built connector-led browser suite. Removing the ServiceNow work-note filter on purpose made the privacy test fetch, store and transfer the private note; restoring it made the test pass. Verification remains fakes-only: no Jira or ServiceNow tenant was contacted, so real visibility metadata, ACLs, pagination and rate limiting remain unobserved.

---

## 2026-09-24 — Backlog: flow efficiency, wait reasons, risk of waiting and cost of delay (Claude)

A product question came up: can Cadena measure non-productive time, explain why work is waiting, and put a price on items sitting too long in a state? Today it can tell you how long an item has been in a state against its SLA, pause the SLA clock in a hold state, and report cycle time, lead time and DORA/ITIL figures. It cannot say how much of an item's life was waiting rather than working, why, how likely a wait is to run long, or what it costs. Nothing was built; the gap is now in the backlog as **Epic 21** (four stories, all not started).

- **US21.1 Flow efficiency:** classify each state as active, waiting or blocked, then report worked versus waiting time. Time in an unclassified state is reported separately, never assumed to be active.
- **US21.2 Wait reasons:** attribute each wait to a reason (customer, third party, dependency, approval, capacity) taken from the hold reason, a blocking link or the source system, and report `unattributed` rather than guess.
...
How much of an item's elapsed time was spent being worked on, and how much waiting? Cadena now answers from the state history it already records.

- **Try it.** Open **Flow efficiency** under Insights. It reports the last 30 days: flow efficiency, active, waiting, blocked and unclassified time, and tables by team, item type and state.
- **Classify states.** At first every state is unclassified, and the dialog says how much time that is. Pick active, waiting or blocked for each state under **Classify workflow states**; the report restates at once. Choose a team to override the organization default for that team.
- **Never guessed.** Time in a state nobody classified counts as elapsed but never as active, and is shown separately. Reclassifying never edits an item's history; each change is a new audited version.
- **Per item.** `GET /workitems/:id/flow-profile` returns each interval with its classification, calendar and minutes. Connector twins use the source system's own timestamps.
- **Know the limits.** Business time follows each state's SLA-policy calendar (24x7 without one). A synced state that no workflow treats as terminal, such as Jira `Done`, keeps counting as unclassified after the item is done. Classification changes are not role-gated. Wait reasons, risk and cost (US21.2 to US21.4) are still to do.

## 2026-09-24 — US17.4 governed bulk synchronization and historical backfill (Claude)

Ordinary polling and scheduled queries follow changes from now on. A migration lead also needs the history: months or years of existing records, brought in as linked twins without hammering Jira or ServiceNow.

- **Try it.** Open **Bulk backfill** in the studio, pick a connector, a start date and an optional end, and **Plan job**. It shows how many chunks the range makes and reads nothing. **Start** hands it to the scheduler; the dialog refreshes itself with a progress bar and the counts until it finishes, and **Download CSV report** exports the audit. The same is available at `POST /integrations/backfill-jobs` and its sub-routes (see the README).
- **Chunks that add up exactly.** The range is split into daily (or hourly, 6-hourly, weekly) windows that share no instant, so every record is read once and a window can be retried alone. A range that would need more than 5,000 chunks is refused with the window size that fits.
- **Resumable, not restartable.** Each page of records commits together with its progress, so pausing, a crash or a restart picks up at the next page. Reading something twice is harmless: the queue drops it. Pause lets the page in flight land; cancel does not.
- **Gentle by design.** A job starts with one chunk at a time and adds one for every three good pages, up to your maximum. If the provider throttles or errors, it halves the concurrency, waits at least as long as `Retry-After` says and tries again. A per-minute ceiling paces every request, and only one job runs per connector at a time.
- **One bad chunk does not sink the job.** A chunk that keeps failing is marked failed and the rest carry on; **Retry failed chunks** resumes it from where it stopped.
- **Counts you can trust.** Processed, queued and failed are read from the real ingestion queue, not a separate tally. The CSV lists every record read (new or already known), the twin it links to, its queue status and any failed chunk. Cells that a spreadsheet could run as a formula are neutralised.
- **A fix that helps everything.** Ingestion now ignores a record older than the twin it would update, so a backfill page read earlier can never roll back something a live sync already brought in.
- **What it is not, yet.** Nothing has run against a live tenant, so real throttling behaviour is unobserved. In a daylight-saving time zone the repeated hour is ambiguous at a chunk edge (UTC is unaffected). Cancel stops further reads but does not undo rows already queued.

**Verified by** `test/us17.4.spec.ts` (35 tests, against the local fakes only; throttle feedback, the pacer, the cancel guard and the stale-version guard were each confirmed to fail when broken), a new browser test and an extended 390px check; 271 non-browser tests across 50 files and 25 browser tests, all passing. `CADENA_BACKFILL_SCHEDULER=disabled` turns the timer off and `CADENA_BACKFILL_TICK_MS` sets its period.

---

## 2026-09-23 — US17.3 scheduled native-query triggers (Claude)

Ordinary polling reads everything a connector is scoped to. A scheduled query lets an integration specialist pick a slice with the provider's own language and have only that slice's changes enqueued, without ever scanning the whole instance.

- **Try it.** Open **Scheduled queries** in the studio, choose a Jira or ServiceNow connector, write `project = CAD AND issuetype = Bug` (or an encoded query such as `assignment_group=<sys_id>^NQcategory=network`), press **Check query**, save the draft, then publish and **Run now**. The same is available at `POST /integrations/native-queries` and its sub-routes (see the README).
- **Unbounded scans are refused, with the fix.** A query such as `status = Open` has no selective scope, so each run would read every issue changed anywhere on the site. Checking it, and publishing it, both fail with the reason and the clause to add (`project = "CAD"`). A scope on only one `OR` branch, or only negations, ranges or text search, does not count. The check is a static heuristic: it cannot see a real instance's indexes.
- **Cadena owns the watermark.** It appends its own `updated >= <watermark>` (or `sys_updated_on>=…`, applied to every `^NQ` part) and the ordering, so a query may not carry its own `ORDER BY` or `updated` filter. The Jira adapter also ANDs the connector's own project scope, so a query can narrow but never widen it. The first run starts at `start_from` (default: publication, at most 366 days back).
- **Durable and de-duplicated.** Matching records enter the same ingestion queue as ordinary polling, under the same dedupe key, and the query's own watermark commits in the same transaction. The connector's polling cursor is untouched, so a re-read of the JQL one-minute overlap enqueues nothing new.
- **Safe to schedule.** An in-process timer runs due queries; each run takes a database lease, so a second instance or a manual run cannot double-run one. A provider failure keeps the watermark, records the error and backs off up to six hours; a run that leaves more pages behind is due again immediately; disabling a query mid-run abandons the run, records included; every run re-validates the stored query.
- **What it is not, yet.** WIQL can be checked but not scheduled, because Cadena has no Azure DevOps adapter. Nothing has run against a live Jira or ServiceNow tenant, so US17.3 is **partial**. The studio does not edit an existing query (the `PATCH` endpoint does).

**Verified by** `test/us17.3.spec.ts` (30 tests, against the local fakes only; two of them were confirmed to fail when their behaviour is broken) and a new browser test plus an extended 390px check; 236 non-browser tests across 49 files and 24 browser tests, all passing. `CADENA_NATIVE_QUERY_SCHEDULER=disabled` turns the timer off and `CADENA_NATIVE_QUERY_TICK_MS` sets its period.

---

## 2026-09-23 — US17.2 governed visual field mappings and field-level write-back (Claude)

State was the only externally-owned field Cadena could write back. Field-level translation is now a first-class, versioned configuration, executed through the same durable pipeline as US13.1 state translation.

- **Mapping contract.** `integration_field_mapping_definitions` holds directional source/target field rules per connector-pair endpoint, each with one transform: direct copy, constant, value table (with an optional default), conditional (branching on any field, including the value being written itself via `$value`), or a sandboxed script. Only the *target* field is checked against the currently discovered schema at publish time — the source field is read from the twin's own already-ingested payload, which legitimately includes derived fields (e.g. Jira's `projectKey`) no discovery endpoint lists. Draft/published/superseded versioning mirrors US13.1 exactly, including SHA-256 schema fingerprints and outbox events on every save and publish.
- **A genuine sandbox, not a policy.** The scripting escape hatch runs in `isolated-vm` — a real, separate V8 engine instance with its own heap, never Node's `vm`/`vm2` (which share the host's heap and event loop and are not a security boundary). Its context starts with zero ambient globals: no `require`, `fetch`, `process` or `global`, so there is nothing to strip. A hard 500ms wall-clock timeout and a 16MB heap limit are enforced by V8 itself; a fresh isolate is created per call, so no state or timing signal leaks between tenants or invocations. `test/mapping-script-sandbox.spec.ts` proves this empirically rather than by assertion: a real infinite loop is actually killed at the timeout, a real memory hog is actually killed at the limit, a generator-constructor-chain escape attempt is blocked, and oversized, malformed, or pathologically deeply-nested (200 levels) return values are all rejected with a specific reason rather than silently truncated or crashing the host.
- **Composite work orders.** `executeWorkOrder` now attempts US13.1 state translation and US17.2 field translation independently and merges whichever actually applies into one work order — a fields-only change (e.g. a priority edit with no state change) never touches state-mapping, and a state-only change never touches field-mapping. Echo suppression's payload now mirrors exactly what a governed write recorded: `state` only when state actually changed, and only the specific fields a published mapping could have written to that endpoint (`writtenFieldNames`), so an unrelated concurrent field change is never mistaken for Cadena's own echo. Schema drift or an unmapped value with no default holds the *whole* composite work order — a partially-applied mapping is never sent.
- **Generalized connector writes.** `ConnectorAdapter.pushUpdate` (renamed from `pushStateChange`) accepts an optional target state, optional fields, or both. Jira sends a field-plus-transition write in one request, or a plain field `PUT` when no state changes. ServiceNow merges fields flat into the same `PATCH` body as a state code. Both sandbox fakes were extended to accept and echo back arbitrary field writes.
- **Governed editing, generalized.** Any canonical field an operator's `writeBack.fields` allow-list names becomes editable in the twin drawer — as a picklist when the provider's discovery reports fixed choices, as free text otherwise (fixing a latent bug: the drawer previously always rendered an empty, unusable `<select>` for a field with no discovered choices). Edits route through the same audited connector work order as a state change and propagate onward through any published field mapping, exactly like a natural change. A new visual field-mapping studio (mirroring the existing state-mapping studio) lets an administrator build value-table and conditional rules or write a script, without hand-authoring JSON.
- **Left open:** a no-write preview panel inside the studio dialog itself (the `/integrations/field-mappings/preview` API exists and is exercised by `test/us17.2.spec.ts`, but the studio UI does not yet call it), and prompting an operator for a translation's `required_target_fields` inline at edit time rather than only at propagation time (both are UI polish, not acceptance-criteria gaps).

**Verified by** `test/us17.2.spec.ts` (11 tests: mapping definitions, publish-time schema and nesting-depth validation, and the full propagation pipeline — fields-only echo suppression, composite state-plus-field work orders, schema-drift holds, and operator field edits), `test/mapping-script-sandbox.spec.ts` (16 tests), 3 new browser tests (a desktop value-table create/publish flow, a 390px layout check of the transform editor, and a free-text drawer edit with its round trip back from a synced connector); 206 non-browser tests across 48 files and 23 browser tests, all passing.
...
- **Replica-safe sync exclusion.** The former process-local set is replaced by an expiring, heartbeated database lease. Staging remains one replica because the audit-chain append path still has an independent single-writer constraint. *(Superseded 2026-09-23: the audit-chain single-writer constraint above was removed; see that entry.)*
...
**Still open after the later queue and projection increments:** only state has an outbound mapping. US16.4/US16.5 and the connector sync lease are now complete; the audit-chain single-writer constraint still prevents multi-replica rollout. *(Superseded 2026-09-23: see the audit-chain fix entry above — `deploy/staging` now runs 2 replicas.)*
